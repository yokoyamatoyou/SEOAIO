...
**目的:** システム全体のデザインをモダンで統一感のあるものに刷新し、ユーザー体験を向上させます。このフェーズでは、apple社風のカラースキームとピクトグラムベースのアイコンを導入し、視覚的な明瞭さを高めます。
...
1. **デザインガイドライン策定:**  
   ...
   * **詳細:**  
     * **カラースキーム:**  
       ...
       * プライマリーカラー: apple社風グレー を主要なアクションボタン、アクティブな要素、強調したい情報に使用します。  
       * セカンダリーカラー/アクセントカラー: apple社を参考に を補足的な情報、一部のアイコン、またはプライマリーカラーとのコントラストが必要な場合に使用します。  
       * テキストカラー: 基本は周囲の色、背景と明確に区別可能なカラーとして、可読性を確保します。エラーメッセージには赤系統、成功メッセージには緑系統の色を定義します。  
     * **タイポグラフィ:**  
       * フォントファミリー: システムフォント（例: Segoe UI, San Francisco, Roboto）または可読性の高いオープンソースフォント（例: Inter, Open Sans）を選定します。  
       * 基本フォントサイズ: 例: 16px。  
       * ヘッダー階層: H1, H2, H3 などのフォントサイズ、ウェイト、マージンを定義します (例: H1: 32px bold, H2: 24px bold)。  フォントのダウンロードがAGENTが可能な場合はAGENTが行い無理な部分はユーザーに簡潔な指示をしてください
     * **アイコン:**  
